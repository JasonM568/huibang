...
function SalarySlip({ r, showSignature }: { r: Record; showSignature: boolean }) {
  const n = (v: string | number) => Number(v) > 0 ? Number(v).toLocaleString() : "";
  const c = "border border-gray-400 px-2 py-0.5";
  ...
  return (
    <div style={{ width: "48%", pageBreakInside: "avoid", fontSize: "11px", lineHeight: "1.4" }}>
      <div className="flex justify-between items-start" style={{ marginBottom: "2px" }}>
        <div>
          <p className="font-bold" style={{ fontSize: "13px" }}>員工薪資表</p>
    ...
    <>
      ...
      <div className="max-w-[210mm] mx-auto bg-white p-6 sm:my-6 sm:shadow-lg print:p-0 print:shadow-none" style={{ fontFamily: "'Noto Sans TC', 'Microsoft JhengHei', sans-serif" }}>
        ...
        <div className="flex justify-between">
          <SalarySlip r={record} showSignature={false} />
          <SalarySlip r={record} showSignature={true} />
        </div>
      </div>
    </>
  );
